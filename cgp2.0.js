    ...
    let APP_CONTAINER;
    			...
    			function applyState(state, isUndoRedo = false) {
    				...
    				try {
    					...
    					if (state.collapsedPalettes && Array.isArray(state.collapsedPalettes)) {
    						...
    						document.querySelectorAll('.cg-palette-section.collapsed').forEach(section => {
    						...
    						paletteOrder.forEach(id => {
    							const palette = document.querySelector(`[data-palette-id="${id}"]`);
    							if (palette) sortedPalettes.push(palette);
    						});
    						let oldCollapsedContainer = document.querySelector('.cg-collapsed-container');
    						if (oldCollapsedContainer) {
    							oldCollapsedContainer.remove();
    						}
    						sortedPalettes.forEach(palette => {
    							container.appendChild(palette);
    						});
    						if (state.collapsedPalettes.length > 0) {
    							const collapsedContainer = document.createElement('div');
    							collapsedContainer.className = 'cg-collapsed-container has-collapsed';
    							container.appendChild(collapsedContainer);
    							state.collapsedPalettes.forEach(paletteId => {
    								const section = document.querySelector(`[data-palette-id="${paletteId}"]`);
    								if (section) {
    									section.classList.add('collapsed');
    									collapsedContainer.appendChild(section);
    									const collapseBtn = section.querySelector('.cg-collapse-button');
    									if (collapseBtn) collapseBtn.textContent = '+';
    								}
    							});
    			...
    			function getPaletteDefaults(paletteType) {
    				const defaults = {
    					...
    					split: {
    						...
    						splitBrightnessShift: 0,  // Add this
    						darkLightPrefs: { dark: 'recommended', light: 'recommended' },
    					...
    					triadic: {
    						...
    						triadicBrightnessMode: 'cascade',  // Add this
    						triadicBrightnessShift: 0,       // Add this
    						darkLightPrefs: { dark: 'recommended', light: 'recommended' },
    			...
    			function setPaletteBackground(paletteSection, color, shouldSaveState = true) {
    				...
    				if (isDarkBg) {
    					...
    					paletteSection.style.setProperty('--dynamic-dl-track-color', '#999999'); // Darker gray track
    			...
    			function createPaletteControls(paletteType) {
    				...
    				if (paletteType === 'monochromatic') {
    					...
    					spreadValue.style.cssText = 'font-size: 12px; color: #545454;';
    					...
    					accentHueValue.style.cssText = 'font-size: 12px; color: #545454;';
    					...
    					accentBrightValue.style.cssText = 'font-size: 12px; color: #545454;';
    					...
    					accentSatValue.style.cssText = 'font-size: 12px; color: #545454;';
    					...
    					rangeValue.style.cssText = 'font-size: 12px; color: #545454;';
    					...
    					distValue.style.cssText = 'font-size: 12px; color: #545454; width: 100%; text-align: center;';
    					...
    					neutralBrightValue.style.cssText = 'font-size: 12px; color: #545454;';
    					...
    					const ratioDropdown = createDropdown(
    						...
    						(e) => { 
    							...
    							if (bottomRow) {
    								const shiftSlider = bottomRow.querySelector('.cg-palette-slider');
    								const shiftGroup = bottomRow.querySelector('.cg-range-with-label');
    								if (warmCoolMode === '2:2') {
    									shiftSlider.classList.remove('shorter');
    									...
    									shiftSlider.classList.add('shorter');
    									shiftGroup.style.flex = '';
    									const spreadGroup = bottomRow.children[1];
    									if (spreadGroup) {
    										spreadGroup.style.display = '';
    									}
    								}
    							}
    							updateColorsAndDisplays(); 
    					...
    					if (warmCoolMode === '2:2') {
    						shiftGroup.style.flex = '1'; // Take full width when alone
    					}
    					...
    					shiftSlider.className = warmCoolMode === '2:2' ? 'cg-palette-slider' : 'cg-palette-slider shorter';
    					shiftSlider.min = '-15';
    					shiftSlider.max = '15';
    					...
    					shiftValue.style.cssText = 'font-size: 12px; color: #545454;';
    					...
    					spreadGroup.className = 'cg-range-with-label';
    					if (warmCoolMode === '2:2') {
    						spreadGroup.style.display = 'none'; // Hide in 2:2 mode
    					}
    					const spreadLabel = document.createElement('div');
    					spreadLabel.className = 'cg-range-label';
    					spreadLabel.textContent = 'Spread';
    					...
    					const spreadSlider = document.createElement('input');
    					spreadSlider.type = 'range';
    					spreadSlider.className = 'cg-palette-slider shorter';
    					spreadSlider.min = '10';
    					spreadSlider.max = '40';
    					spreadSlider.value = warmCoolSpread;
    					spreadSlider.dataset.default = '20';
    					const spreadValue = document.createElement('span');
    					spreadValue.textContent = warmCoolSpread + '°';
    					spreadValue.style.cssText = 'font-size: 12px; color: #545454;';
    					spreadSlider.addEventListener('input', (e) => {
    						warmCoolSpread = parseInt(e.target.value);
    						spreadValue.textContent = warmCoolSpread + '°';
    						updateColorsAndDisplays();
    					});
    					spreadSlider.addEventListener('change', saveState);
    					...
    					balanceValue.style.cssText = 'font-size: 12px; color: #545454;';
    					...
    					tensionValue.style.cssText = 'font-size: 12px; color: #545454;';
    					...
    					slider.type = 'range';
    					slider.className = 'cg-palette-slider shorter'; // Use shorter class
    					slider.min = '20';
    					slider.max = '60';
    					...
    					value.style.cssText = 'font-size: 12px; color: #545454;';
    					...
    					const brightnessSliderGroup = document.createElement('div');
    					brightnessSliderGroup.className = 'cg-palette-slider-group';
    					const brightnessSlider = document.createElement('input');
    					brightnessSlider.type = 'range';
    					brightnessSlider.className = 'cg-palette-slider shorter';
    					brightnessSlider.min = '-50';
    					brightnessSlider.max = '50';
    					brightnessSlider.value = splitBrightnessShift;
    					brightnessSlider.dataset.default = '0';
    					const brightnessValue = document.createElement('span');
    					brightnessValue.style.cssText = 'font-size: 12px; color: #545454; min-width: 45px; text-align: center;';
    					const updateBrightnessLabel = (val) => {
    						val = parseInt(val);
    						if (val === 0) brightnessValue.textContent = 'None';
    						else if (val < 0) brightnessValue.textContent = `−${Math.abs(val)}%`;
    						else brightnessValue.textContent = `+${val}%`;
    					};
    					brightnessSlider.addEventListener('input', (e) => {
    						splitBrightnessShift = parseInt(e.target.value);
    						updateBrightnessLabel(splitBrightnessShift);
    						updateColorsAndDisplays();
    					});
    					brightnessSlider.addEventListener('change', saveState);
    					updateBrightnessLabel(splitBrightnessShift);
    					brightnessSliderGroup.appendChild(brightnessSlider);
    					brightnessSliderGroup.appendChild(brightnessValue);
    					brightnessGroup.appendChild(brightnessLabel);
    					brightnessGroup.appendChild(brightnessSliderGroup);
    					...
    					const vibranceDropdownElement = createDropdown(
    						[
    							...
    							{value: 'cascade', text: 'Cascade'},
    							{value: 'primary-muted', text: 'Pri+Muted'},
    							{value: 'duotone-accent', text: 'Duo+Acc'}
    						],
    						triadicVibrance,
    					...
    					const brightShiftSliderGroup = document.createElement('div');
    					brightShiftSliderGroup.className = 'cg-palette-slider-group';
    					const brightShiftSlider = document.createElement('input');
    					brightShiftSlider.type = 'range';
    					brightShiftSlider.className = 'cg-palette-slider shorter';
    					brightShiftSlider.min = '-50';
    					brightShiftSlider.max = '50';
    					brightShiftSlider.value = triadicBrightnessShift;
    					brightShiftSlider.dataset.default = '0';
    					const brightShiftValue = document.createElement('span');
    					brightShiftValue.style.cssText = 'font-size: 12px; color: #545454; min-width: 45px; text-align: center;';
    					const updateBrightLabel = (val) => {
    						val = parseInt(val);
    						if (val === 0) brightShiftValue.textContent = 'None';
    						else if (val < 0) brightShiftValue.textContent = `−${Math.abs(val)}%`;
    						else brightShiftValue.textContent = `+${val}%`;
    					};
    					brightShiftSlider.addEventListener('input', (e) => {
    						triadicBrightnessShift = parseInt(e.target.value);
    						updateBrightLabel(triadicBrightnessShift);
    						updateColorsAndDisplays();
    					});
    					brightShiftSlider.addEventListener('change', saveState);
    					updateBrightLabel(triadicBrightnessShift);
    					brightShiftSliderGroup.appendChild(brightShiftSlider);
    					brightShiftSliderGroup.appendChild(brightShiftValue);
    					brightShiftGroup.appendChild(brightShiftLabel);
    					brightShiftGroup.appendChild(brightShiftSliderGroup);
    					...
    					dominanceValue.style.cssText = 'font-size: 12px; color: #545454;';
    			...
    			function generateColorPalettes() {
    				...
    				container.innerHTML = '';
    				const palettes = [
    					{ title: 'Monochromatic', desc: 'Single hue palette with varied brightness and saturation levels for subtle, sophisticated harmony', colors: generateMonochromatic(), type: 'monochromatic' },
    					{ title: 'Dominant + Accents', desc: 'Primary color with supporting secondary tone and vibrant accent for balanced visual hierarchy', colors: generateDominantAccent(), type: 'dominant' },
    					{ title: 'Neutral + Pop', desc: 'Muted neutral foundation with strong vibrant accent for focused attention and elegance', colors: generateNeutralPop(), type: 'neutralpop' },
    					{ title: 'Analogous', desc: `Adjacent colors within ${analogousRange}° on the color wheel for natural, harmonious flow`, colors: generateAnalogous(), type: 'analogous' },
    					{ title: 'Complementary', desc: 'Opposite colors on the color wheel creating maximum contrast, energy, and visual impact', colors: generateComplementary(), type: 'complementary' },
    					{ title: 'Warm/Cool Split', desc: 'Strategic contrast between warm and cool temperature zones for dynamic visual tension', colors: generateWarmCoolSplit(), type: 'warmcool' },
    					{ title: 'Split Complementary', desc: 'Base color paired with two colors adjacent to its complement for vibrant yet harmonious contrast', colors: generateSplitComplementary(), type: 'split' },
    					{ title: 'Triadic', desc: 'Three evenly spaced colors forming a triangle on the color wheel for vibrant, balanced energy', colors: generateTriadic(), type: 'triadic' },
    					{ title: 'Tetradic', desc: 'Four colors forming two complementary pairs for rich, complex palettes with maximum variety', colors: generateTetradic(), type: 'tetradic' }
    				];
    				pinnedPaletteElements = [];
                ...
                function updatePaletteDisplay(section, colorData, paletteType) {
    				...
    				if (paletteType === 'analogous') {
    					...
    					if (titleEl) {
    						titleEl.setAttribute('data-description', `Adjacent colors within ${analogousRange}° on the color wheel for natural, harmonious flow`);
    					}
    				}
                    updateAllColorText();
                }
    			...
    			function updateUIFromState() {
    				...
    				document.querySelectorAll('.cg-palette-section').forEach(section => {
    					...
    					if (paletteType === 'monochromatic') {
    						...
    						if (bottomRow) {
    							const shiftGroup = bottomRow.children[0];
    							const spreadGroup = bottomRow.children[1];
    							updateSlider(shiftGroup, temperatureShift, v => `${v >= 0 ? '+' : ''}${v}°`);
    							if (warmCoolMode === '2:2') {
    								if (spreadGroup) spreadGroup.style.display = 'none';
    								const shiftSlider = shiftGroup.querySelector('.cg-palette-slider');
    								if (shiftSlider) {
    									shiftSlider.classList.remove('shorter');
    								}
    								shiftGroup.style.flex = '1';
    								...
    								if (spreadGroup) {
    									spreadGroup.style.display = '';
    									updateSlider(spreadGroup, warmCoolSpread, v => `${v}°`);
    								}
    								const shiftSlider = shiftGroup.querySelector('.cg-palette-slider');
    								if (shiftSlider) {
    									shiftSlider.classList.add('shorter');
    						...
    						const rows = controls.querySelectorAll('.cg-palette-controls-row');
    						const angleSlider = rows[0].querySelector('.cg-palette-slider');
    						if (angleSlider) {
    							updateSlider(rows[0].children[0], splitAngle, v => `${v}°`);
    						}
    						if (rows[0].children[1]) {
    							const brightSlider = rows[0].children[1].querySelector('.cg-palette-slider');
    							if (brightSlider) {
    								brightSlider.value = splitBrightnessShift;
    								const brightValue = rows[0].children[1].querySelector('span');
    								if (brightValue) {
    									if (splitBrightnessShift === 0) brightValue.textContent = 'None';
    									else if (splitBrightnessShift < 0) brightValue.textContent = `−${Math.abs(splitBrightnessShift)}%`;
    									else brightValue.textContent = `+${splitBrightnessShift}%`;
    								}
    							}
    						}
    						...
    						updateDropdown(rows[0].children[1], triadicVibrance);
    						if (rows[1]) {
    							updateDropdown(rows[1].children[0], triadicBrightnessMode);
    							updateSlider(rows[1].children[1], triadicBrightnessShift, v => `${v}%`);
    						}
    					} else if (paletteType === 'tetradic') {
    						const rows = controls.querySelectorAll('.cg-palette-controls-row');
            ...
            APP_CONTAINER = document.querySelector('.cgp-container') || document.body;
    			...
    			document.addEventListener('DOMContentLoaded', function() {
    				...
    				function initChangelogPopup() {
    					...
    					overlay.style.display = 'none';
    					const changelogHTML = `
    						<h3>Version 2.0 - ${new Date().toLocaleDateString()}</h3>
    						<ul>
